...
const RUNTIME = `(function () {
  ...
  function swatchOf(el) {
    ...
    var pal = el.closest('.pal');
    if (pal) return { sw: pal, colorEl: pal.querySelector('.block') || pal, hxEl: pal.querySelector('.hx'), token: pal.getAttribute('data-role-token') };
  ...
  function applyRoleFont(role, font) {
    ensureFont(font);
    var r = String(role || '').toUpperCase();
    var tok = /^(H1|H2|H3|HEADING|DISPLAY|TITLE)$/.test(r) ? '--font-heading' : '--font-body';
    ROOT.style.setProperty(tok, '"' + font + '", system-ui, sans-serif');
    var card = DOC.querySelector('[data-font="' + tok + '"] .fam-name'); if (card) card.textContent = font;
    dirty();
  }
  ...
  function swatchTok(el) {
    var sw = el.closest && el.closest('.pal, .sw'); if (!sw) return null;
    var nm = sw.querySelector('.nm'); var label = nm ? nm.textContent.toLowerCase() : '';
    // new role model: the swatch carries its token + category directly
    var roleTok = sw.getAttribute('data-role-token');
    var token = roleTok
      || (label.indexOf('brand') >= 0 || label.indexOf('primary') >= 0 ? '--color-brand'
      : label.indexOf('secondary') >= 0 ? '--color-secondary'
      : label.indexOf('accent') >= 0 ? '--color-accent'
      : label.indexOf('background') >= 0 || label.indexOf('page') >= 0 ? '--color-surface-page'
      : label.indexOf('border') >= 0 ? '--color-border'
      : label.indexOf('card') >= 0 ? '--color-surface-card'
      : label.indexOf('alt') >= 0 ? '--color-surface-alt'
      : label.indexOf('text') >= 0 ? '--color-text'
      : label.indexOf('muted') >= 0 ? '--color-muted' : '');
    if (!token) return null;
    return { sw: sw, token: token, cat: sw.getAttribute('data-role-cat') || 'text', role: nm ? nm.textContent.trim() : 'Color' };
  }
  // The brand-palette roles a semantic token can point at.
  var PALETTE_ROLES = [['Primary', '--color-brand'], ['Secondary', '--color-secondary'], ['Accent', '--color-accent'], ['Text', '--color-text'], ['Background', '--color-surface-page'], ['Border', '--color-border']];
  // "Applies to" targets → the SEMANTIC token each usage drives (so a brand colour can be
  // assigned to Headings, Buttons, etc.). Consistent with the Semantic Tokens grid.
  var COLOR_TARGETS = [
    { label: 'Headings (H1–H6)', tok: '--text-heading' },
    { label: 'Body text', tok: '--text-primary' },
    { label: 'Captions / muted', tok: '--text-muted' },
    { label: 'Links', tok: '--link-color' },
    { label: 'Buttons (fill)', tok: '--btn-bg' },
    { label: 'Button text', tok: '--btn-text' },
    { label: 'Alt sections', tok: '--color-surface-alt' },
    { label: 'Page background', tok: '--color-surface-page' },
    { label: 'Borders & dividers', tok: '--color-border' }
  ];
  // refresh the little preview squares in the Semantic Tokens grid (var() repaints, this is a no-op safety)
  function refreshSemSquares() { try { DOC.querySelectorAll('.sem .sq').forEach(function (q) { q.style.display = q.style.display; }); } catch (e) {} }
  // clean checkbox list: tick = point that usage at this brand colour (var(--role));
  // a colour set directly on an element still wins (override).
  function appliesToRows(roleToken) {
    var wrap = DOC.createElement('div'); wrap.style.cssText = 'margin-top:10px;border-top:1px solid #eef0f4;padding-top:10px';
    var cap = DOC.createElement('div'); cap.textContent = 'Applies to'; cap.style.cssText = 'font-size:11px;font-weight:700;color:#7a7f8c;text-transform:uppercase;letter-spacing:.05em;margin-bottom:8px'; wrap.appendChild(cap);
    var ref = ('var(' + roleToken + ')').replace(/\\s+/g, '');
    COLOR_TARGETS.forEach(function (tg) {
      var r = DOC.createElement('div'); r.style.cssText = 'display:flex;align-items:center;gap:9px;padding:5px 0;cursor:pointer';
      var cb = DOC.createElement('input'); cb.type = 'checkbox'; cb.style.cssText = 'width:15px;height:15px;flex:0 0 auto;margin:0;cursor:pointer;accent-color:#473AE0';
      cb.checked = (getTokRaw(tg.tok) || '').replace(/\\s+/g, '') === ref;
      var sp = DOC.createElement('span'); sp.textContent = tg.label; sp.style.cssText = 'font-size:12.5px;font-weight:500;color:#3a3f4c;text-transform:none;letter-spacing:0;text-align:left';
      cb.addEventListener('change', function () { if (cb.checked) setTok(tg.tok, 'var(' + roleToken + ')'); else { ROOT.style.removeProperty(tg.tok); dirty(); } refreshSemSquares(); });
      r.addEventListener('click', function (e) { if (e.target !== cb) { cb.checked = !cb.checked; cb.dispatchEvent(new Event('change')); } });
      r.appendChild(cb); r.appendChild(sp); wrap.appendChild(r);
    });
    return wrap;
  }
  // Clean Add / Delete for the brand palette — appends a NEW swatch with its own unique
  // token (no DOM-clone that shares a token, which caused the duplicate-swatch bug).
  function paletteActions(sw) {
    var bar = DOC.createElement('div'); bar.style.cssText = 'display:flex;gap:8px;margin-top:12px;border-top:1px solid #eef0f4;padding-top:10px';
    var add = DOC.createElement('button'); add.type = 'button'; add.textContent = '+ Add color';
    add.style.cssText = 'flex:1;border:1px solid #e2e2ec;background:#f7f7fb;border-radius:8px;padding:8px;font-size:12px;font-weight:600;cursor:pointer;color:#473AE0';
    add.onclick = function (ev) {
      ev.stopPropagation();
      var grid = sw.parentElement; if (!grid) return;
      var tok = '--color-custom-' + Date.now().toString(36);
      setTok(tok, '#8B8B8B');
      var d = DOC.createElement('div'); d.className = 'pal'; d.setAttribute('data-role-token', tok); d.setAttribute('data-copy', '#8B8B8B');
      d.innerHTML = '<div class="block" style="background:var(' + tok + ')"></div><div class="nm">New color</div><div class="hx">#8B8B8B</div><div class="use">Custom color</div>';
      grid.appendChild(d); dirty(); closePop();
    };
    bar.appendChild(add);
    var rt = sw.getAttribute('data-role-token') || '';
    if (rt.indexOf('--color-custom-') === 0) {
      var del = DOC.createElement('button'); del.type = 'button'; del.textContent = 'Delete';
      del.style.cssText = 'border:1px solid #f1d4d4;background:#fdf2f2;border-radius:8px;padding:8px 14px;font-size:12px;font-weight:600;cursor:pointer;color:#c0392b';
      del.onclick = function (ev) { ev.stopPropagation(); sw.remove(); dirty(); closePop(); };
      bar.appendChild(del);
    }
    pop.appendChild(bar);
  }
  ...
  // Editable "Used by" note — shown in every Properties popover, persisted on the clicked
  // element (data-sg-use) so it saves with the guide.
  function usedBy(text) {
    var i = DOC.createElement('input'); i.type = 'text';
    i.value = (target && target.getAttribute && target.getAttribute('data-sg-use')) || text || '';
    i.placeholder = 'e.g. Buttons \\u00b7 Links';
    i.addEventListener('input', function () { if (target && target.setAttribute) target.setAttribute('data-sg-use', i.value); dirty(); });
    pop.appendChild(row('Used by', i));
  }

  /* ---- element-aware token rows: read current value from the token if set, else from the
     clicked element's computed style (so the field shows the REAL current value), write to
     the per-element token so it applies live to every element of that kind ---- */
  function curVal(token, cssProp, fbToken) {
    var v = getTokRaw(token); if (v) return v;
    if (cssProp) { try { var c = cs(target, cssProp); if (c && c !== 'normal' && c !== 'auto') return c; } catch (e) {} }
    if (fbToken) { var f = getTokRaw(fbToken); if (f) return f; }
    return '';
  }
  function elColorRow(label, token, cssProp, fbToken) {
    return colorField(label, function () { return rgbToHex(curVal(token, cssProp, fbToken) || '#000000'); }, function (v) { setTok(token, v); });
  }
  function elTextRow(label, token, cssProp, ph, fbToken) {
    var i = DOC.createElement('input'); i.type = 'text'; i.value = curVal(token, cssProp, fbToken).trim(); i.placeholder = ph || '';
    i.addEventListener('input', function () { setTok(token, i.value); });
    return row(label, i);
  }
  function elSelRow(label, token, cssProp, opts, fbToken) {
    var cur = (curVal(token, cssProp, fbToken) || opts[0]).trim();
    return selField(label, cur, opts, function (v) { setTok(token, v); });
  }
  function elFontRow(label, token, cssProp) {
    return fontButton(label, function () { return (curVal(token, cssProp) || '').split(',')[0].replace(/["']/g, '').trim(); },
      function (f) { ensureFont(f); setTok(token, '"' + f + '", system-ui, sans-serif'); });
  }
  // All font families defined in the Font Families panel → [displayName, token].
  function fontChoices() {
    var out = [];
    DOC.querySelectorAll('[data-font]').forEach(function (f) {
      var tok = f.getAttribute('data-font');
      // show the ACTUAL font name (from the token value), not the card label
      var name = (getTokRaw(tok) || '').split(',')[0].replace(/["']/g, '').trim();
      if (!name) { var nm = f.querySelector('.fam-name'); name = nm ? nm.textContent.trim() : tok; }
      out.push([name, tok]);
    });
    if (!out.length) out = [['Inter', '--font-heading'], ['Inter', '--font-body']];
    return out;
  }
  // Per-element font: choose only among the CHOSEN families (added in the Font Families panel) —
  // the actual fonts are picked there. defaultRole = which one applies if unset.
  function elFontChoiceRow(label, token, defaultRole) {
    var choices = fontChoices();
    var opts = choices.map(function (c) { return c[0]; });
    var cur = (getTokRaw(token) || '').replace(/\\s+/g, '');
    var sel = '';
    choices.forEach(function (c) { if (cur === 'var(' + c[1] + ')') sel = c[0]; });
    if (!sel) { var defTok = defaultRole === 'body' ? '--font-body' : '--font-heading'; choices.forEach(function (c) { if (c[1] === defTok) sel = c[0]; }); }
    if (!sel) sel = opts[0];
    return selField(label, sel, opts, function (v) { var ch = choices.filter(function (c) { return c[0] === v; })[0]; if (ch) setTok(token, 'var(' + ch[1] + ')'); });
  }
  // Add / Delete font families in the Font Families panel.
  function fontActions(famEl) {
    var bar = DOC.createElement('div'); bar.style.cssText = 'display:flex;gap:8px;margin-top:12px;border-top:1px solid #eef0f4;padding-top:10px';
    var add = DOC.createElement('button'); add.type = 'button'; add.textContent = '+ Add font';
    add.style.cssText = 'flex:1;border:1px solid #e2e2ec;background:#f7f7fb;border-radius:8px;padding:8px;font-size:12px;font-weight:600;cursor:pointer;color:#473AE0';
    add.onclick = function (ev) {
      ev.stopPropagation();
      var rowEl = famEl.parentElement; if (!rowEl) return;
      // normalise the row + give every card a consistent card look (works even on older guides).
      rowEl.style.display = 'grid'; rowEl.style.gap = '24px';
      rowEl.querySelectorAll('.fam').forEach(function (c) { c.style.border = '1px solid rgba(0,0,0,.1)'; c.style.borderRadius = '12px'; c.style.padding = '24px'; c.style.background = '#fff'; });
      var tok = '--font-custom-' + Date.now().toString(36);
      ensureFont('Inter'); setTok(tok, '"Inter", system-ui, sans-serif');
      var d = DOC.createElement('div'); d.className = 'fam'; d.setAttribute('data-font', tok);
      d.style.cssText = 'padding:24px;border:1px solid rgba(0,0,0,.1);border-radius:12px;cursor:pointer;background:#fff';
      d.innerHTML = '<div class="fam-name" style="font-family:var(' + tok + ');font-size:34px;font-weight:700;line-height:1.1;color:var(--text-heading)">Inter</div>'
        + '<div class="fam-use" style="font-size:13px;color:var(--text-muted);margin-top:12px">Custom font</div>'
        + '<div class="fam-weights" style="margin-top:12px;display:flex;gap:16px;flex-wrap:wrap;color:var(--text-muted);font-size:14px"><span style="font-weight:400">Regular</span><span style="font-weight:600">SemiBold</span><span style="font-weight:700">Bold</span></div>';
      rowEl.appendChild(d);
      rowEl.style.gridTemplateColumns = 'repeat(' + Math.min(rowEl.querySelectorAll('.fam').length, 3) + ', 1fr)'; // max 3 per row
      dirty(); closePop();
    };
    bar.appendChild(add);
    var ft = famEl.getAttribute('data-font') || '';
    if (ft.indexOf('--font-custom-') === 0) {
      var del = DOC.createElement('button'); del.type = 'button'; del.textContent = 'Delete';
      del.style.cssText = 'border:1px solid #f1d4d4;background:#fdf2f2;border-radius:8px;padding:8px 14px;font-size:12px;font-weight:600;cursor:pointer;color:#c0392b';
      del.onclick = function (ev) { ev.stopPropagation(); var rw = famEl.parentElement; famEl.remove(); if (rw) rw.style.gridTemplateColumns = 'repeat(' + Math.min(rw.querySelectorAll('.fam').length, 3) + ', 1fr)'; dirty(); closePop(); };
      bar.appendChild(del);
    }
    pop.appendChild(bar);
  }

  // Properties mode is STRICT: only the design-system elements from the spec are editable —
  // brand colours, semantic tokens, type specimens, the spacing/container token markers,
  // radius boxes, links/lists/quotes, buttons, form fields, breakpoints. Nothing else
  // (page title, section descriptions, component demos, cards, nav, footer) responds.
  function isEditableProp(el) {
    if (!el || !el.closest) return false;
    if (el.closest('.tmeta')) return false;   // type-specimen size label is read-only (edit size on the heading itself)
    if (el.closest('[data-typ]') || el.closest('[data-radius]') || el.closest('[data-sem-token]') || el.closest('[data-font]')) return true;
    if (el.closest('.pal') || el.closest('.sw') || el.closest('.logo-slot')) return true;
    if (el.closest('#breakpoints')) return true;
    if (el.closest('.page-head') || el.closest('.section-head')) return false;
    if (el.closest('.specimen') || el.closest('.tspec')) return true; // type specimens (H1–H6, Body, Caption)
    if (el.closest('.btn')) return true;                      // buttons
    if (el.tagName === 'INPUT' || el.tagName === 'TEXTAREA' || el.tagName === 'SELECT') return true; // form fields
    if (el.closest('#content')) { var tg = el.tagName.toLowerCase(); if (/^(a|ul|ol|li|blockquote|p)$/.test(tg)) return true; } // links/lists/quotes
    return false;
  }
  ...
  function openProps(el) {
    ...
    // Logo slot: upload a logo image (stored as a data URL in the guide)
    var logoEl = el.closest && el.closest('.logo-slot');
    if (logoEl) {
      var limg = logoEl.querySelector('img');
      header('Logo');
      var lfi = DOC.createElement('input'); lfi.type = 'file'; lfi.accept = 'image/*'; lfi.style.display = 'none';
      lfi.addEventListener('change', function () { var f = lfi.files && lfi.files[0]; if (!f) return; var rd = new FileReader(); rd.onload = function () { if (limg) limg.setAttribute('src', String(rd.result)); dirty(); }; rd.readAsDataURL(f); });
      var lup = DOC.createElement('button'); lup.type = 'button'; lup.textContent = '\\u2191 Upload logo';
      lup.style.cssText = 'width:100%;border:1px solid #e2e2ec;background:#f7f7fb;border-radius:8px;padding:9px;font-size:12px;font-weight:600;cursor:pointer;color:#473AE0;margin-bottom:8px';
      lup.onclick = function (e) { e.stopPropagation(); lfi.click(); };
      pop.appendChild(lup); pop.appendChild(lfi);
      var capEl = logoEl.querySelector('.logo-cap'); usedBy(capEl ? capEl.textContent.trim() : '');
      position(el); return;
    }
    // Font-family card: pick the actual font (searchable Google Fonts) for heading or body
    var famEl = el.closest && el.closest('[data-font]');
    if (famEl) {
      var ftok = famEl.getAttribute('data-font');
      header(ftok === '--font-heading' ? 'Heading font' : 'Body font');
      pop.appendChild(fontButton('Font family', function () { return (getTokRaw(ftok) || '').split(',')[0].replace(/["']/g, '').trim(); },
        function (f) { ensureFont(f); setTok(ftok, '"' + f + '", system-ui, sans-serif'); var nm = famEl.querySelector('.fam-name'); if (nm) nm.textContent = f; }));
      var useEl = famEl.querySelector('.fam-use'); usedBy(useEl ? useEl.textContent.trim() : '');
      fontActions(famEl);
      position(el); return;
    }
    // Semantic token: choose WHICH brand colour drives this usage (Headings, Links, Button fill…)
    var semEl = el.closest && el.closest('[data-sem-token]');
    if (semEl) {
      var semTok = semEl.getAttribute('data-sem-token');
      var dsEl = semEl.querySelector('.ds');
      header('Token \\u00b7 ' + (dsEl ? dsEl.textContent.trim() : semTok));
      var curRaw = (getTokRaw(semTok) || '').replace(/\\s+/g, '');
      var names = PALETTE_ROLES.map(function (r) { return r[0]; });
      var curName = 'Custom';
      PALETTE_ROLES.forEach(function (r) { if (curRaw === ('var(' + r[1] + ')')) curName = r[0]; });
      pop.appendChild(selField('Uses brand color', curName, names, function (v) {
        var role = PALETTE_ROLES.filter(function (r) { return r[0] === v; })[0];
        if (role) { setTok(semTok, 'var(' + role[1] + ')'); var sq = semEl.querySelector('.sq'); if (sq) sq.style.background = 'var(' + role[1] + ')'; }
      }));
      usedBy(dsEl ? dsEl.textContent.trim() : '');
      position(el); return;
    }
    var sw = swatchTok(el);
    if (sw) {
      header('Brand color \\u00b7 ' + sw.role);
      pop.appendChild(tokColorRow('Color (hex)', sw.token, function (v) {
        var hx = sw.sw.querySelector('.hx'); if (hx) hx.textContent = v.toUpperCase(); sw.sw.setAttribute('data-copy', v);
      }));
      var useEl = sw.sw.querySelector('.use');
      var di = DOC.createElement('input'); di.type = 'text'; di.value = useEl ? useEl.textContent.trim() : ''; di.placeholder = 'Description';
      di.addEventListener('input', function () { if (useEl) { useEl.textContent = di.value; dirty(); } });
      pop.appendChild(row('Description', di));
      pop.appendChild(appliesToRows(sw.token));   // choose where this colour applies (grouped)
      paletteActions(sw.sw);                       // clean Add / Delete (no DOM-clone bug)
      position(el); return;
    }
    if (/^h[1-6]$/.test(tag)) {
      var hp = '--' + tag; // --h1 .. --h6
      header('Typography \\u00b7 ' + tag.toUpperCase());
      pop.appendChild(elFontChoiceRow('Font', hp + '-font-family', 'heading'));
      pop.appendChild(elTextRow('Font size', hp + '-font-size', 'font-size', '48px'));
      pop.appendChild(elSelRow('Font weight', hp + '-font-weight', 'font-weight', WEIGHTS));
      pop.appendChild(elTextRow('Line height', hp + '-line-height', '', '1.2'));
      pop.appendChild(elTextRow('Letter spacing', hp + '-letter-spacing', '', '-0.02em'));
      pop.appendChild(elColorRow('Color', hp + '-color', 'color'));
      pop.appendChild(elTextRow('Margin top', hp + '-margin-top', '', '0'));
      pop.appendChild(elTextRow('Margin bottom', hp + '-margin-bottom', '', '24px'));
      pop.appendChild(elSelRow('Text transform', hp + '-text-transform', 'text-transform', ['none', 'uppercase', 'lowercase', 'capitalize']));
      position(el); return;
    }
    ...
    if (tag === 'p' && !(el.closest && el.closest('blockquote'))) {
      header('Typography \\u00b7 Paragraph');
      pop.appendChild(elFontChoiceRow('Font', '--p-font-family', 'body'));
      pop.appendChild(elTextRow('Font size', '--p-font-size', 'font-size', '16px'));
      pop.appendChild(elSelRow('Font weight', '--p-font-weight', 'font-weight', WEIGHTS));
      pop.appendChild(elTextRow('Line height', '--p-line-height', '', '1.7'));
      pop.appendChild(elColorRow('Color', '--p-color', 'color'));
      pop.appendChild(elTextRow('Margin bottom', '--p-margin-bottom', '', '16px'));
      pop.appendChild(elTextRow('Max width', '--p-max-width', '', '65ch'));
      position(el); return;
    }
    if (tag === 'a' && !(el.classList && el.classList.contains('btn'))) {
      header('Links');
      pop.appendChild(elFontChoiceRow('Font', '--a-font-family', 'body'));
      pop.appendChild(elColorRow('Color', '--a-color', 'color', '--link-color'));
      pop.appendChild(elSelRow('Text decoration', '--a-text-decoration', 'text-decoration-line', ['none', 'underline', 'overline', 'line-through'], '--link-decoration'));
      pop.appendChild(elSelRow('Font weight', '--a-font-weight', 'font-weight', WEIGHTS));
      pop.appendChild(elColorRow('Color (hover)', '--a-color-hover', '', '--link-color-hover'));
      pop.appendChild(elSelRow('Decoration (hover)', '--a-text-decoration-hover', '', ['none', 'underline', 'overline', 'line-through'], '--link-decoration-hover'));
      pop.appendChild(elTextRow('Transition', '--a-transition', '', 'color 0.15s ease'));
      position(el); return;
    }
    if (el.classList && el.classList.contains('btn')) {
      header('Button');
      pop.appendChild(elFontChoiceRow('Font', '--btn-font-family', 'body'));
      pop.appendChild(elTextRow('Font size', '--btn-font-size', 'font-size', '16px'));
      pop.appendChild(elSelRow('Font weight', '--btn-weight', 'font-weight', WEIGHTS));
      pop.appendChild(elTextRow('Letter spacing', '--btn-letter-spacing', '', '0.02em'));
      pop.appendChild(elSelRow('Text transform', '--btn-text-transform', 'text-transform', ['none', 'uppercase', 'lowercase', 'capitalize']));
      ...
      pop.appendChild(tokColorRow('Background (hover)', '--btn-bg-hover'));
      pop.appendChild(tokColorRow('Text (hover)', '--btn-text-hover'));
      pop.appendChild(elTextRow('Transition', '--btn-transition', '', 'background 0.2s ease'));
      position(el); return;
    }
    if (tag === 'input' || tag === 'textarea' || tag === 'select') {
      header('Form field');
      pop.appendChild(elTextRow('Font size', '--input-font-size', 'font-size', '16px'));
      pop.appendChild(elSelRow('Font weight', '--input-font-weight', 'font-weight', WEIGHTS));
      pop.appendChild(elTextRow('Line height', '--input-line-height', '', '1.5'));
      pop.appendChild(elTextRow('Letter spacing', '--input-letter-spacing', '', '0em'));
      pop.appendChild(tokColorRow('Text color', '--input-text'));
      pop.appendChild(tokColorRow('Placeholder color', '--input-placeholder-color'));
    ...
    var listEl = el.closest && el.closest('ul, ol');
    if (listEl && !(el.closest && el.closest('blockquote'))) {
      var isOl = listEl.tagName.toLowerCase() === 'ol';
      var lp = isOl ? '--ol' : '--ul';
      header(isOl ? 'Ordered List' : 'Unordered List');
      pop.appendChild(elFontChoiceRow('Font', lp + '-font-family', 'body'));
      pop.appendChild(elSelRow('List style', lp + '-list-style-type', 'list-style-type', isOl ? ['decimal', 'decimal-leading-zero', 'lower-alpha', 'upper-alpha', 'lower-roman', 'upper-roman'] : ['disc', 'circle', 'square', 'none']));
      pop.appendChild(elTextRow('Padding left', lp + '-padding-left', 'padding-left', '24px'));
      pop.appendChild(elTextRow('Margin bottom', lp + '-margin-bottom', '', '16px'));
      pop.appendChild(elTextRow('Item line height', lp + '-li-line-height', '', '1.6'));
      pop.appendChild(elTextRow('Item margin bottom', lp + '-li-margin-bottom', '', '8px'));
      position(el); return;
    }
    if (tag === 'blockquote' || (el.closest && el.closest('blockquote'))) {
      header('Blockquote');
      pop.appendChild(elFontChoiceRow('Font', '--bq-font-family', 'body'));
      pop.appendChild(elTextRow('Font size', '--bq-font-size', 'font-size', '18px'));
      pop.appendChild(elSelRow('Font style', '--bq-font-style', 'font-style', ['normal', 'italic']));
      pop.appendChild(elSelRow('Font weight', '--bq-font-weight', 'font-weight', WEIGHTS));
      pop.appendChild(elTextRow('Line height', '--bq-line-height', '', '1.6'));
      pop.appendChild(elColorRow('Color', '--bq-text-color', 'color'));
      pop.appendChild(elTextRow('Border width', '--bq-border-left-width', 'border-left-width', '4px'));
      pop.appendChild(elColorRow('Border color', '--bq-border-left-color', 'border-left-color'));
      pop.appendChild(elTextRow('Padding left', '--bq-padding-left', 'padding-left', '24px'));
      pop.appendChild(elTextRow('Margin', '--bq-margin', '', '32px 0'));
      position(el); return;
    }
    ...
    // No specific token handler matched → in strict Properties mode, do nothing.
    closePop();
  }
  ...
  DOC.addEventListener('click', function (e) {
    ...
    if (MODE === 'properties') { if (!isEditableProp(t)) { closePop(); return; } openProps(t); post('section', { sectionId: sectionOf(t) }); return; }
  ...
  DOC.addEventListener('mouseover', function (e) { var t = e.target; if (isUi(t) || (t.closest && t.closest('.tocnav'))) return; if (MODE === 'properties' && !isEditableProp(t)) return; if (t !== target && t !== BODY && t !== ROOT) t.classList.add('sg-hov'); }, true);
  ...
  window.__sg = {
    ...
    // update a brand-palette swatch hex label/copy (the chip uses var(), auto-updates).
    // name is the colour token, e.g. --color-brand.
    setPalette: function (name, hex) {
      if (!/^#[0-9a-fA-F]{6}$/.test(hex)) return;
      var sw = DOC.querySelector('.pal[data-role-token="' + name + '"], .sw[data-role-token="' + name + '"]');
      if (!sw) return;
