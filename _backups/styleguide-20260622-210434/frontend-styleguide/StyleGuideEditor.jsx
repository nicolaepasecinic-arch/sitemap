...
import { ArrowLeft, Download, Share2, X, Check, Loader2, Copy, Users, ChevronDown, Settings, Bot, LogOut, Save, Plus, Trash2, History, Sparkles, Send } from 'lucide-react';
...
const SECTIONS = [
  { id: 'colors', n: '01', label: 'Colors' },
  { id: 'typography', n: '02', label: 'Typography' },
  { id: 'spacing', n: '03', label: 'Spacing' },
  { id: 'radii', n: '04', label: 'Radius' },
...
export default function StyleGuideEditor({ id, user, onBack, onLogout, onUserChange, embedded = false }) {
  ...
  const applyAiResult = (out) => {
    ...
    // map the brand-palette colour tokens to their visible swatch
    const PALETTE = { '--color-brand': 'brand', '--color-accent': 'accent', '--color-bg-light': 'surfaceAlt', '--color-warm': 'surfaceCard', '--color-white': 'surfacePage', '--color-muted': 'muted' };
    (out.tokenUpdates || []).forEach(({ name, value }) => {
      let v = value;
      if (/font/i.test(name) && v && v.indexOf(',') < 0) { api.loadFont(v); v = '"' + v + '", sans-serif'; }
      api.setToken(name, v);
      const role = PALETTE[name.toLowerCase()];
      if (role && api.setPalette && /^#[0-9a-fA-F]{6}$/.test(value)) api.setPalette(role, value);
    });
    ...
    <div className={`${embedded ? 'absolute' : 'fixed'} inset-0 flex flex-col bg-[#FBFCFE]`}>
      ...
      <div className="h-14 shrink-0 bg-white border-b border-gray-100 flex items-center px-4">
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          {/* mode switcher — editor only, kept left so it's always visible */}
          {!readOnly && (
            <div className="inline-flex items-center bg-gray-100 rounded-full p-0.5 shrink-0 ml-1" title="Properties: edit design tokens (they propagate everywhere). Everything: edit any element freely.">
        ...
        <div className="flex justify-center shrink min-w-0 overflow-x-auto no-scrollbar">
