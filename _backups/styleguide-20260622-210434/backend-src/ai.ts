...
function prettifyFont(v: any, fallback: string): string {
  ...
  // drop variable-font / weight technical suffixes
  s = s.replace(/[-_\s]?(variable|var|vf|variablefont|standard|regular|text|display|web|webfont)$/gi, '').trim();
  // normalise separators → spaces, then Title Case each word
...
export const STYLE_PROBE_JS = `(function(){
  ...
  // Buttons = elements with a fill/border + padding; radius = most common.
  function btnLike(e){try{var s=getComputedStyle(e);var bg=s.backgroundColor;var hasBg=bg&&bg!=='rgba(0, 0, 0, 0)'&&bg!=='transparent';var hasBorder=(num(s.borderTopWidth)||0)>0;return vis(e)&&(hasBg||hasBorder)&&(num(s.paddingLeft)||0)>=8;}catch(e){return false;}}
  var btns=all('button, a, [role="button"], input[type="submit"], input[type="button"], .btn, [class*="button"]').filter(btnLike);
  var btnRad=mode(btns.map(function(e){return num(getComputedStyle(e).borderRadius);}).filter(function(x){return x!=null;}));
  var filled=btns.filter(function(e){var bg=getComputedStyle(e).backgroundColor;return bg&&bg!=='rgba(0, 0, 0, 0)'&&bg!=='transparent';}).sort(function(a,b){var ra=a.getBoundingClientRect(),rb=b.getBoundingClientRect();return (rb.width*rb.height)-(ra.width*ra.height);});
  var bf=filled[0],bfc=bf?getComputedStyle(bf):null;
  ...
  // Sections = full-width blocks; vertical padding = median of real values.
  var iw=window.innerWidth||1280;
  var spads=all('section, main > div, [class*="section"]').filter(function(e){if(!vis(e))return false;var r=e.getBoundingClientRect();return r.width>=iw*0.6;}).map(function(e){return num(getComputedStyle(e).paddingTop)||0;}).filter(function(x){return x>=24&&x<=400;});
  var sectionY=median(spads);
  ...
  return {
    ...
    colors:{heading:(H&&H.color)||bcs.color,text:bcs.color,pageBg:bcs.backgroundColor,link:link?getComputedStyle(link).color:'',btnBg:bfc?bfc.backgroundColor:'',btnColor:bfc?bfc.color:''},
...
export function tokensFromComputed(branding: any, computed: any): Partial<SgTokens> | null {
  ...
  let headingHex = rgbToHexStr(cc.heading) || rgbToHexStr(cc.text);
  if (headingHex && lum(headingHex) > 0.6) headingHex = ''; // too light for a light page → drop
  const accentHex = rgbToHexStr(cc.btnBg) || rgbToHexStr(cc.link) || rgbToHexStr(b.accent) || rgbToHexStr(b.link);
  ...
  const out: Partial<SgTokens> = {
    colors: {
      // brand = the dark heading/text colour (NOT the primary, which is often the violet accent)
      brand: headingHex || rgbToHexStr(b.textPrimary) || rgbToHexStr(b.primary) || '#1F2937',
      // accent = the REAL button/link colour so it stays distinct from brand (buttons, links only)
      accent: rgbToHexStr(cc.btnBg) || rgbToHexStr(cc.link) || rgbToHexStr(b.accent) || rgbToHexStr(b.link) || accentHex || '#2563EB',
    ...
    spacing: {
      sectionY: roundPx(sp.sectionY) || '96px', sectionYTablet: '72px', sectionYMobile: '56px',
      containerWidth: roundPx(sp.containerWidth) || '1200px', containerPadding: '24px',
    },
  };
  return out;
}
...
export async function generateStyleGuideTokens(opts: { url: string; instructions?: string; branding?: any; screenshotUrl?: string }): Promise<SgTokens> {
  ...
  return {
    ...
    colors: { brand, accent, surfaceAlt, surfaceCard, surfacePage, muted },
    headingFont,
    bodyFont,
    type: {
...
const SG_ASSIST_PROMPT = `You are the assistant inside a DESIGN-SYSTEM / style-guide editor. The whole look is driven by CSS custom properties (design tokens) on :root — colours (--color-*, --surface-*, --text-*, --btn-*, --border-*), fonts (--font-serif, --font-sans), type sizes (--text-h1…--text-h6, --text-body-*), line-heights (--lh-*), letter-spacing (--ls-*), weights (--fw-*) and spacing (--space-*, --gap-*, radii).
...
BRAND PALETTE TOKENS (these drive the big visible swatches — ALWAYS set them when changing the colour scheme):
- --color-brand   = primary brand colour (headlines, nav, dark UI)
- --color-accent  = accent / action colour (buttons, links, highlights)
- --color-bg-light = a light tinted alternate surface
- --color-warm    = a soft card surface
- --color-white   = the page background (usually #FFFFFF)
- --color-muted   = a neutral grey for captions
Also update the matching semantic tokens (--surface-*, --text-*, --btn-*, --border-*) for consistency. When the user asks to match a website or change the palette, you MUST return concrete #RRGGBB values for all six brand palette tokens (do not leave them as the old plum/orange).

Return:
- reply: one short, friendly sentence describing what you changed.
- tokenUpdates: ONLY the tokens you are changing, each as { name (the exact --token), value }. Colours must be #RRGGBB. Fonts must be real Google Fonts family names (quoted stack not needed — just the family). Sizes keep their unit (e.g. "120px", "1.2", "-0.03em").
- roleFonts: optional per-role font assignments for headings/body, each { role (one of H1,H2,H3,H4,H5,H6,Body,Buttons,Nav), font (Google Fonts family) } — use this when the user asks to change a specific heading/body font.

Change as much or as little as the instruction needs. Keep good contrast and readability. Do not invent token names that aren't in the provided list unless adding an obviously-standard one.`;
